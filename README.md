...
based on the library [nepali_datetime](https://github.com/amitgaru/nepali-datetime).

_The APIs documentation URL is https://ndt.amitgaru.com.np/docs._
...
1. To get today's Bikram Sambat Date
   ...
   ```sh
   curl -X GET https://ndt.amitgaru.com.np/date
   ...
   ```sh
   curl -X GET "https://ndt.amitgaru.com.np/date?format=%d-%B-%y"
   # returns { "data": "26-Aswin-77" }
   ```

   **More about the formatting in the table described [here](https://amitgaru.github.io/nepali-datetime/html/index.html#strftime-and-strptime-behavior).**

1. Similarly, to get today's Bikram Sambat Date & current Nepal Time

   ```sh
   curl -X GET https://ndt.amitgaru.com.np/datetime
...
1. Convert Bikram Sambat (BS) date to AD date
   ...
   ```sh
   curl -X GET 'https://ndt.amitgaru.com.np/date/convert/bs-ad?bs_date=2080-10-29&format=%Y-%m-%d'
...
1. Convert AD date to Bikram Sambat (BS) date
   ...
   ```sh
   curl -X GET 'https://ndt.amitgaru.com.np/date/convert/ad-bs?ad_date=2024-02-12&format=%Y-%m-%d'
